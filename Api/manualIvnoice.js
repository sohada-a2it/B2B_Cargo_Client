...
export const getMyManualInvoices = async (params = {}) => {
    try {
        const queryParams = new URLSearchParams();
        ...
        const url = `/my-manual-invoices${queryParams.toString() ? `?${queryParams.toString()}` : ''}`;
        const response = await axiosInstance.get(url);
        ...
        return {
            ...
            message: error.response?.data?.message || error.message || 'Failed to fetch invoices',
            data: [],
            summary: { total: 0, paid: 0, pending: 0, totalAmount: 0 },
            pagination: { total: 0, page: 1, limit: 20, pages: 0 }
        };
    }
};