...
// Api/booking.js - trackByNumber ফাংশন

export const trackByNumber = async (trackingNumber) => {
    try {
        // ক্যাশ বাইপাস করতে timestamp যোগ করুন
        const timestamp = Date.now();
        
        const response = await axiosInstance.get(`/track/${trackingNumber}?_=${timestamp}`, {
            headers: {
...
// Calculate totals from cargo details (legacy)
...
// Calculate totals from package details (new)
...
// Format cargo details for display (legacy)
...
// Format package details for display (new)
...
export const exportBookingsToCSV = (bookings, filename = 'bookings.csv') => {
  ...
  const csvData = bookings.map(booking => [
    ...
    getOriginDisplay(booking.shipmentDetails?.origin) || booking.sender?.address?.country || 'N/A',
    getDestinationDisplay(booking.shipmentDetails?.destination) || booking.receiver?.address?.country || 'N/A',
    getShipmentTypeDisplay(booking.shipmentDetails?.shipmentType),
    getShippingModeDisplay(booking.shipmentDetails?.shippingMode),
    booking.shipmentDetails?.totalPackages || booking.shipmentDetails?.totalCartons || 0,
